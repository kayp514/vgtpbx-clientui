...
export function SidebarContent() {
  ...
  const dialplanItems = navigation.filter(
    (item) => item.section === "dialplan"
  );
  const voiceItems = navigation.filter((item) => item.section === "voice");
  const messagingItems = navigation.filter(
    (item) => item.section === "messaging"
  );
  const siptrunksItems = navigation.filter(
    (item) => item.section === "siptrunks"
  );
  ...
  const renderSection = (
    sectionItems: typeof navigation,
    sectionKey: string
  ) => {
    ...
    const hasActiveItem = sectionItems.some((item) =>
      item.submenu?.some(
        (subItem) => subItem.href && isActiveSection(subItem.href)
      )
    );
    ...
    return (
      <div
        key={sectionKey}
        className={cn("mb-1", isCollapsed ? "px-0.5" : "px-1")}
      >
        <Button
        ...
        >
          ...
          >
            ...
            {!isCollapsed && (
              <span className="text-sm font-medium">{sectionData.name}</span>
            )}
          </div>
          {!isCollapsed && (
            <motion.div
        ...
        >
          <div
            className={cn("flex flex-col gap-0.5", !isCollapsed && "pl-2 pr-1")}
          >
      ...
      <SidebarFooter className="border-t bg-gradient-to-b from-muted/10 to-muted/30">
        {userId && (
          <SidebarUserFooter
            user={{
              id: userId,
              name: user?.displayName || sessionClaims?.email || "User",
              email: user?.email || sessionClaims?.email || "",
