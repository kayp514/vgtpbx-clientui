...
const POLLING_INTERVALS = {
  pending: 5000,      // 5 seconds when pending
  provisioning: 3000, // 3 seconds when actively provisioning
  provisioned: 0,     // No polling when done
  failed: 0,          // No polling when failed
} as const;

export function ProvisioningStatusIndicatorNew() {
  const { pbxDomain, isChecking, refreshStatus } = useProvisioning();
  ...
  // Setup polling based on current status
  const currentStatus = pbxDomain?.status;
  
  useEffect(() => {
    if (!currentStatus) return;

    const interval = POLLING_INTERVALS[currentStatus] ?? 0;
  ...
  }, [currentStatus, startPolling, clearPolling]);
